...
class GeometryKeeper:
	...
	def make_fracture_geometry(self, gdict, xd, yd, xwd, ywd, N, c, ndigits):
		#N = well.params["nseg"]
		dx = 1./N
		#xwd = well.xwd
		#c = well.c
		...
		gdict["mxd"] = m_xd
		gdict["myd"] = np.matrix((yd - ywd)*np.ones_like(m_xd))
		gdict["lims1"] = m_xd - c*m_xj1 - xwd # lower limits of integration within fracture: lims1[i, j] = dx*(i+0.5) - dx*(j-1)
		gdict["lims2"] = m_xd - c*m_xj2 - xwd # upper limits of integration within fracture: lims2[i, j] = dx*(i+0.5) - dx*j
		...
		gdict["alims1"] = np.round(np.abs(gdict["lims1"]), decimals=ndigits) # absolute values of lims1 rounded to ndigits
		gdict["alims2"] = np.round(np.abs(gdict["lims2"]), decimals=ndigits) # absolute values of lims2 rounded to ndigits
		gdict["ulims"] = np.unique([np.round(gdict["alims1"], decimals=ndigits), np.round(gdict["alims2"], decimals=ndigits)]) # unique values of limits for speedup
		gdict["xcs"] = xwd + dx*np.matrix(np.arange(-N+0.5, N+0.5), dtype=np.float) # centers of segments within fracture (as array)
		gdict["xends"] = xwd + dx*np.arange(-N, N+1, dtype = np.float)