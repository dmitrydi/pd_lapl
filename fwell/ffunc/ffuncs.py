...
def ih1F2H(zd, zwd, x1, x2, u, ksid, ksiwd, ksied, ksiede, hd, yd, ywd, buf, fid):
    ...
    for i in range(MAXITER):
        d = np.zeros(mshape, dtype=np.double)
        for n in np.arange(1+i*blk_size, 1+(i+1)*blk_size):
            d += ih1F2Hn(n, zd, zwd, x1, x2, u, ksid, ksiwd, ksied, ksiede, hd, yd, ywd, buf, fid)
        sum_ += d
        ns = np.linalg.norm(sum_) 
        if np.linalg.norm(d) < EPS*ns or ns < TINY :
            return sum_
...
def ih1F2Hn(n, zd, zwd, x1, x2, u, ksid, ksiwd, ksied, ksiede, hd, yd, ywd, buf):
    PI = np.pi
    return np.cos(n*PI*zd)*np.cos(n*PI*zwd)*i1F2H(x1, x2, u, ksid, ksiwd, ksied, ksiede, n*PI/hd, yd, ywd, buf, fid)
